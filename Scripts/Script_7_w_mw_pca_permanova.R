# ============================================================
# Script 6 
# pt. 1 PCA of 9 physicochemical properties including molecular weight
# ============================================================
# Purpose:
...
#
# The aim of this script is to run a PCA using nine physicochemical properties,
# including molecular weight. 
# The nine physicochemical properties used in the PCA are:
# - Molecular Weight (Da)
# - cLogP
# - cLogD at pH 7.4
...
# Make Figure S4.3:
# Scree plot showing the proportion of variance explained by the first nine
# principal components and the cumulative variance explained.

make_scree_plot <- function(eig_all, N = 9) {
...
# Set working directory for PCA/PERMANOVA with molecular weight

setwd(here("Files", "PCA_PERMANOVA_W_MW_Figs"))

# Save Figure S4.3  as a TIFF.
tiff(
  "Figure_S4_3_scree_plot_w_MW.tiff",
  width = 7,
  height = 5,
  units = "in",
  res = 600,
...
## ---------- 1.8) Extract PCA scores and save PCA summary table Table S4.1. ----------
...
load <- scores(PCA, display = "species", choices = 1:9)

# Get a summary of the PCA.
summary(PCA)

# Make Table S4.1:
# PCA eigenvalues and variance explained for PC1-PC9.
...
write_xlsx(
  pca_summary_df,
  "Table_S4_1_pca_summary_w_MW.xlsx"
)
...
# Make biplots for:
#
# - Figure S4.5: PC1 vs PC2
# - Figure S4.6: PC1 vs PC3
# - Figure S4.7: PC2 vs PC3
...
# Save Figure S4.5: PC1 vs PC2.
tiff(
  "Figure_S4_5_PCA_PC1_PC2_w_MW.tiff",
  width = 7,
  height = 6,
  units = "in",
  res = 600,
...
# Save Figure S4.6: PC1 vs PC3.
tiff(
  "Figure_S4_6_PCA_PC1_PC3_w_MW.tiff",
  width = 7,
  height = 6,
  units = "in",
  res = 600,
...
# Save Figure S4.7: PC2 vs PC3.
tiff(
  "Figure_S4_7_PCA_PC2_PC3_w_MW.tiff",
  width = 7,
  height = 6,
  units = "in",
  res = 600,
...
# Make Figure S4.4:
# Heatmap of species scores for PC1-PC9.

# Extract species scores for PC1-PC9.
...
PCA_species_scores_heatmap_PC19 <- 
...
# Save Figure S4.4 as a TIFF.
# 300-600 dpi is typical for publication/Word outputs.
setwd(here("Files", "PCA_PERMANOVA_W_MW_Figs"))

ggsave(
  filename = "Figure_S4_4_PCA_species_scores_heatmap_w_MW.tiff",
  plot = PCA_species_scores_heatmap_PC19,
  device = "tiff",
  width = 7,
  height = 6,
  units = "in",
...
ggsave(
  filename = "Basic_group_PC1_2_labs_w_MW.tiff",
...
ggsave(
  filename = "Basic_group_PC1_3_labs_w_MW.tiff",
...
#Figure S4.8: PC1 vs PC2, coloured and shaped by class 
...
## ----------  1.19) Save Figure S4.8 & S4.9 ----------
setwd(here("Files", "PCA_PERMANOVA_W_MW_Figs"))

# Save Figure S4.8 as a TIFF.
ggsave(
  filename = "Figure_S4_8_PC1_PC2_w_MW.tiff",
...
# Save Figure S4.9 as a TIFF.
ggsave(
  filename = "Figure_S4_9_PC1_PC3_w_MW.tiff",
...
#
# Figure S4.1:
# PCA plot, PC1 vs PC2, using nine physicochemical properties:
# Molecular weight hydrogen bond donor count, hydrogen bond acceptor count, cLogD7.4, cLogP,
...
#
# Figure S4.2:
# PCA plot, PC1 vs PC3, using the same nine physicochemical properties and design 
#
...
## ploy S4.1: PC1 vs PC2 coloured by priority group 

# PCA plot of nine physicochemical properties, PC1 vs PC2.
...
# Plot Figure  S4.2.
# PCA plot of nine physicochemical properties, PC1 vs PC3.
# Points represent compounds and are coloured by prioritisation group.
# Antibiotics are shaped by antibiotic class, while pollutants are shown as
# triangles.
...
## ---------- 1.23) Save Figure S4.1. & S4.2. ----------
setwd(here("Files", "PCA_PERMANOVA_W_MW_Figs"))

# Save Figure 1: PC1 vs PC2, coloured by priority group.
ggsave(
  filename = "Figure_S4_1_w_MW.tiff",
...
# Save Figure S4.2.: PC1 vs PC3, coloured by priority group.
ggsave(
  filename = "Figure_S4_2_PC1_PC3_w_MW.tiff",
...
# ============================================================
# pt. 2 PERMANOVA of 9 physicochemical properties excluding molecular weight
# for Supplementary File 4 
# ============================================================
#
# Purpose:
#
...
#
# The nine physicochemical properties used in the analysis are:
#
# - Molecular Weight (Da)
# - cLogP
# - cLogD at pH 7.4
...
setwd(here("Files", "PCA_PERMANOVA_W_MW_Results"))
...
## ---------- 2.8) Define pairwise betadisper contrasts ----------
...
# <chr>                                   <dbl>   <dbl> <chr>   
# 1 Group_1_pollutant vs Group_1_antibiotic 11.2   0.0101 "*"     
# 2 Group_2_pollutant vs Group_2_antibiotic  1.72  0.233  ""      
# 3 Group_3_pollutant vs Group_3_antibiotic  6.03  0.0669 "."

#Permutation-based tests of multivariate dispersion showed significant difference in dispersion between antibiotics and pollutants in Priority Group 1 (F =11.20, adjusted p = 0.01*), but not in Group 2 (F = 1.72, adjusted p = 0.23) and Group 3 (F = 6.03, adjusted p = 0.07). This suggests that the PERMANOVA results for Priority Group 1 may be influenced by differences in multivariate dispersion, while the results for Priority Groups 2 and were not primarily driven by significant dispersion differences.
...
# Model      7   924.08 0.47099 26.837 9.999e-05 ***
# Residual 211  1037.92 0.52901                     
# Total    218  1962.00 1.00000                     
# ---
...
# 1 Group_1_pollutant vs Group_1_antibiotic     0.215     10.9    0.000255 ***              1            38.3
# 2 Group_2_pollutant vs Group_2_antibiotic     0.347     10.1    0.000800 ***              1            94.2
# 3 Group_3_pollutant vs Group_3_antibiotic     0.674     22.7    0.00122  **               1            22.2

#Pairwise PERMANOVA of nine physicochemical properties (molecular weight, hydrogen bond donor count, hydrogen bond acceptor count, cLogD₇.₄, cLogP, relative PSA, rotatable bond count, globularity score and net charge at pH 7.4), showed significant differences between antibiotics and pollutants in Group 1 (R2 = 0.22, pseudo-F = 10.94, adjusted p = 0.0003***), Group 2 (R2 = 0.35 , pseudo-F = 10.08, adjusted p = 0.0008***), and Group 3 (R2 = 0.67, pseudo-F = 22.70 , adjusted p = 0.0012**). However, Group 1 and Group 2 showed weaker separation between antibiotics and pollutants than Group 3, based on their smaller PERMANOVA effect sizes (R2 = 0.22 and 0.35 vs 0.67). 
...
# This will be used in Supplementary File 4 
write_xlsx(
  list(
  ...
  "PERMANOVA_betadisper_results_w_mw.xlsx"
)
